...
export default function CampaignDetailScreen({ campaign, onSelectVtuber, onBack }) {
  const [period, setPeriod] = useState('30d');
  const sorted = [...campaign.vtubers].sort((a, b) => b.clicks - a.clicks);
  const barData = sorted.map(v => ({ name: v.name, clicks: v.clicks }));
  const daily = campaign.daily.filter((_, i) => i % 2 === 0);
  ...
  return (
    <div style={{ padding: '28px 32px', flex: 1, overflowY: 'auto', background: '#f0f9ff' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
            <h1 style={{ fontSize: 19, fontWeight: 700, color: '#0f172a' }}>{campaign.name}</h1>
            <StatusBadge status={campaign.status} />
          </div>
          <p style={{ fontSize: 12, color: '#64748b', marginBottom: 2 }}>{campaign.description}</p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 20 }}>
        <MetricCard label="総クリック数" value={campaign.totalClicks.toLocaleString()} sub="案件全体" primary />
        <MetricCard label="参加VTuber" value={campaign.vtubers.length} sub="名" />
        <MetricCard
          label="平均/VTuber"
          value={Math.round(campaign.totalClicks / campaign.vtubers.length).toLocaleString()}
          sub="クリック"
        />
      </div>
        ...
        <div style={{ background: '#fff', border: '1px solid #e0f2fe', borderRadius: 10, padding: 18 }}>
          ...
          <ResponsiveContainer width="100%" height={190}>
            <BarChart data={barData} layout="vertical">
              ...
              <YAxis dataKey="name" type="category" tick={{ fontSize: 9, fill: '#64748b' }} width={68} />
              <Tooltip contentStyle={tt} />
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          <thead>
            <tr style={{ background: '#f8fafc' }}>
              {['順位', 'VTuber名', 'クリック数', 'シェア', '前期比', ''].map((h, i) => (
                <th key={i} style={{
          ...
          <tbody>
            {sorted.map((v, i) => {
              const share = ((v.clicks / campaign.totalClicks) * 100).toFixed(1);
              return (
                <tr key={v.id} style={{ borderTop: '1px solid #f0f9ff' }}
                ...
                >
                  ...
                  <td style={{ padding: '12px 18px', textAlign: 'center' }}>
                    <span style={{ fontSize: 12, fontWeight: 600, color: v.trend >= 0 ? '#16a34a' : '#dc2626' }}>
